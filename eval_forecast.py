        ...
        for label, key in COUNTRIES.items():
            ...
            for horizon in HORIZONS:
                for param in PARAMS:
                    ...
                    m_prophet = backtest_prophet(df, param, horizon_days=horizon, train_window_days=TRAIN_WINDOW_DAYS)
                    m_naive   = backtest_naive_persistence(df, param, horizon_days=horizon, train_window_days=TRAIN_WINDOW_DAYS)

                    rows.append({"country": label, "param": param, "horizon_days": horizon, "model": "Prophet", **m_prophet})
