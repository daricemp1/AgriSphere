...
# Evaluate forecasting performance across all country/parameter/horizon combinations.
# Returns a DataFrame of metrics for each model (N-BEATS and NaivePersistence).
def evaluate_all() -> pd.DataFrame:
    # Accumulate per-run metric dicts here; we'll convert to a DataFrame at the end.
    rows = []
    # Compute total number of iterations for a complete progress bar (countries × horizons × params).
    total_iterations = len(COUNTRIES) * len(HORIZONS) * len(PARAMS)
    
    # Create a single progress bar tracking the overall loop progress.
    with tqdm(total=total_iterations, desc="Overall Progress", ncols=100) as pbar:
        # Iterate over each configured country (label shown in results; key used to fetch data).
        for label, key in COUNTRIES.items():
            # Console breadcrumb to separate countries in stdout logs.
            print(f"\nProcessing {label}...")
            # Pull historical weather (and soil moisture) for this country; feeds all backtests below.
            ...
            for horizon in HORIZONS:
                # Loop over each weather parameter to evaluate 
                for param in PARAMS:
                    # Update progress bar title with current triplet for live feedback.
                    pbar.set_description(f"{label} - {param} - {horizon}d")
                    
                    # Guard: if this parameter is entirely missing/NaN for the dataset, record NaNs and skip modeling.
                    if df[param].isna().all():
                        # Placeholder metrics for N-BEATS when no data exists.
                        rows.append({
                            "country": label,
                            "param": param,
                        ...
                        # Same placeholder metrics for the naive baseline for fair tabulation.
                        rows.append({
                            "country": label,
                            "param": param,
                            "horizon_days": horizon,
                        ...
                        # Advance overall progress and move to next (param/horizon/country).
                        pbar.update(1)
                        continue

                    # Attempt the N-BEATS backtest; if a run fails, capture and record NaNs (keeps batch robust).
                    try:
                        # Train/test split via rolling backtest for the given horizon and training window.
                        m_nbeats = backtest_nbeats(df, param, horizon_days=horizon, train_window_days=TRAIN_WINDOW_DAYS)
                    except Exception as e:
                        # Surface the error for debugging but don't halt the overall evaluation loop.
                        print(f"Error with N-BEATS for {label}-{param}-{horizon}d: {e}")
                        # Fallback metrics for failed runs to preserve table shape.
                        m_nbeats = {"MAE": float("nan"), "RMSE": float("nan"), "MAPE%": float("nan"), "R2": float("nan"), "n_tests": 0}
                    
                    # Always run the naive (persistence) baseline for comparison under the same settings.
                    m_naive = backtest_naive_persistence(df, param, horizon_days=horizon, train_window_days=TRAIN_WINDOW_DAYS)

                    # Record N-BEATS results with identifying metadata columns.
                    rows.append({"country": label, "param": param, "horizon_days": horizon, "model": "N-BEATS", **m_nbeats})
                    # Record naive baseline results with the same identifiers for side-by-side analysis.
                    rows.append({"country": label, "param": param, "horizon_days": horizon, "model": "NaivePersistence", **m_naive})

                    # Mark this (country, param, horizon) combination as completed on the progress bar.
                    pbar.update(1)
    
    # Consolidate all metric dicts into a tidy DataFrame and sort for stable, readable output.
    results = pd.DataFrame(rows).sort_values(["country", "param", "horizon_days", "model"]).reset_index(drop=True)
    # Return the final evaluation table to the caller 
    return results
