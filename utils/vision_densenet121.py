...
# Builds Densenet121 model with custom classifier layer
def create_disease_model(num_disease_classes):
    weights = models.DenseNet121_Weights.DEFAULT
    model = models.densenet121(weights=weights) 
    model.classifier = torch.nn.Linear(model.classifier.in_features, num_disease_classes)
    return model
...
def load_vision_model():
    ...
    model.load_state_dict(torch.load("densenet121_disease.pt", map_location="cpu"))
    model.eval()
